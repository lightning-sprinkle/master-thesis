...
=======
Fourthly, the lnd module handles the communication with the lightning network node. This communication is based on gRPC. It provides a function that is able to send a payment to an address immediately using the keysend method. The keysend method is described in more detail in section \ref{} (werkende link?). In order to authenticate at the lnd service, macaroons are used. Macaroons are comparable to cookies and contain authentication data. They are implemented in such a way that different types of macaroons provide different levels of privileges. 
>>>>>>> a5531ac... Update 04_implementation.tex
